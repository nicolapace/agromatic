...
class Sending_goal():
    ...
    def __init__(self):
        ...
        self._weight_sub = rospy.Subscriber('weight_toBase', Weight , self.sub_weight_callback)
        self._weight_pub = rospy.Publisher('weight_data', Weight, queue_size=10)
    ...
    #callback del topic del peso
    def sub_weight_callback(self, msg):
        self._weight_data = msg

    def weightUpdate(self):
        w = Weight()
        w.Reset=False
        w.ToBase=False
        w.nFruits =  self.nFruits
        self._weight_pub.publish(w)
